...
def group_add(groupname):
    """Create new group"""
    grp = bpy.data.collections.new(groupname)
    return grp
...
def delete_all():
    ...
    for object_name in candidate_list:
        bpy.data.objects[object_name].select_set(True)            # Rhett 20210526 Raw: .select = True
...
def view_selected_group(gname):
    ...
    bpy.ops.object.select_same_collection(collection=gname)
    bpy.ops.view3d.view_selected()
