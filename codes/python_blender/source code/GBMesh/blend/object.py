...
def icosphere_add(name, size=1, origin=(0, 0, 0), smooth=True):
    ...
    bpy.ops.mesh.primitive_ico_sphere_add(radius=size, location=origin)     # Rhett changed here: size to radius, 20210524
...
def link_and_update(obj):
    ...
    scene.collection.objects.link(obj)                          # Modified by Rhett 20210521 Add .collection
    bpy.context.view_layer.objects.active = obj                 # Modified by Rhett 20210521 original: scene.objects.active = obj
    obj.select_set(True)                                        # MOdified by Rhett 20210521 original: obj.select = True 
    bpy.context.view_layer.update()                             # Modified by Rhett 20210521 original: scene.update() 
    return obj
...
def delete(obj):
    ...
    obj.select_set(state=True)                                         # Rhett changed 20210521  obj.select = True
    bpy.context.view_layer.objects.active = obj                        # Rhett changed 20210521 scene.objects.active = obj
...
def dupli_set(obj, type):
    ...
    obj.instance_type = type                               # Rhett changed 20210524 obj.dupli_type = type 
...
def parent_set(parent, child):
    ...
    bpy.context.view_layer.objects.active  = parent     # Rhett chanaged 202105214 bpy.context.scene.objects.active = parent
    child.select_set(True)                              # Rhett changed 20210524 child.select = True

    # sets selected object as child of active object
    bpy.ops.object.parent_set(type='OBJECT', keep_transform=False)

    # add child object to parent object's groups
    for grp in parent.users_collection:                     # Rhett changed 20210524 for grp in parent.users_group:
        space.group_add_object(grp, child)

def select(obj, sel):
    """Select or deselect object. Boolean sel."""
    obj.select_set(state=sel)                                        # Rhett changed 20210524 obj.select = sel 
