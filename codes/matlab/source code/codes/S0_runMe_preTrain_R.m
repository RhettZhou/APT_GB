...
TestPer = 0.05;
numOfGBnetTest = ceil(TestPer*numOfGBnet);

numOfNoise = 1;  %%% 4                                                     % For the same GB strucutre, how many different solute distribution can have.
filterFactor = 1.8;                                                        % determine the threshold to file out atoms, used in function proj22D

GBnetSize = 330;  %%% 400                                                  % Atomic structure setup, unit Angstrom
GBnetHeight = 50; %%% 50                                                   % Height of unit box
GBnNetGrainNum = 3; %%% 3                                                  % How many grains inside the box, periodic condiction

resolu2D = 3;   %%% 4                                                      % Resolution in 2D images, unit Angstrom
subSizeNm = 60;  %%% 80                                                    % Subimage size in unit Angstrom
imGap = 30; %%% 16
...
imGapSize = imGap/resolu2D;                                                % 10 pixel
numOfImgPerUnit = ((GBnetSize-subSizeNm)/imGap+1)^2;                       % 100 images

%%
for i = numOfGBnetRestar:numOfGBnet+numOfGBnetTest                       % There will be one extra for the training dataset.
...
toc
  for j = 1:numOfNoise
    ...
    cluRad = 5 + round(GBNetWidth*0.2*rand);                               % cluster radius in the bulk
    ...
    GBcluRad = 7 + round(GBNetWidth*0.4*rand);                             % cluster radius in the GBs
    ...
    numOfImgPerSub = subImage(conProjMS,subSize,ifGB,grainInd,pathName,group,imGapSize);     % Cut and save subImages, generate labels
    toc
  end
end

collectImg_mlGB(segment,numOfGBnet,numOfNoise,numOfImgPerUnit,numOfGBnetTest,pathName);
