%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [GBVal,Sfv] = smoothValue_R(pathName,atomIon)
if ~exist('pathName','var')
    pathName = uigetdir({},'Select a folder');
end

fPathName = [pathName '\maps\'];
aPathName = [pathName '\values\'];

mkdir(pathName,'\s_maps')
...
while isfile([fPathName '\' checkName(num2str(i)) '_C_raw.ply'])
    fv = IEC(fPathName, i);
    fv.Cs = smooth_R(fv,fv.Cs)';
    fv.IEs = smooth_R(fv,fv.IEs)';
    
    fileID = fopen([aPathName '\' checkName(num2str(i)) '_data.txt'],'r');
    aIEC = fscanf(fileID,formatSpec,aSize);
    fclose(fileID);
    
    aIEC = aIEC';
    
    [IEAve,IEErr,CAve,CErr,AreaGB] = saveGB(checkName(num2str(i)),aIEC,fv.IEs,fv.Cs,fv,pathName,atomIon);
    
    if i ~= 0
        GBVal(:,i) = [i,IEAve,IEErr,CAve,CErr,AreaGB];
    else
        Sfv = fv;
    end
...
f6 = figure('Name','Interfacial excess and concentration values after smoothing','Visible','Off');
yyaxis left
...
xlim([min(GBVal(1,:))-1,max(GBVal(1,:))+1])
hold off
fileNameOut = [pathName '\statistics\s_statistics.fig'];
set(f6, 'CreateFcn', 'set(gcbo,''Visible'',''On'')');
saveas (f6, fileNameOut);
%% Export the statistic results
IEData = [pathName '\statistics\s_statistic.txt'];
csvwrite(IEData,GBVal');
end

function fv = IEC(pathName, GB)
plyN_C = [pathName '\' checkName(num2str(GB)) '_C_raw.ply'];
plyN_IE = [pathName '\' checkName(num2str(GB)) '_IE_raw.ply'];
fvC = geneFV(plyN_C);
fvIE = geneFV(plyN_IE);
...
function [IEAve,IEErr,CAve,CErr,AreaGB] = saveGB(GBID,aIEC,IEmap,Cmap,interface,pathName,atomIon)
%% export to *.ply  IEmap
AreaGB = sum(aIEC(:,1));

minIE = min(IEmap);
maxIE = max(IEmap);
IEAve = sum(IEmap.*aIEC(:,1))/AreaGB;
IEErr = std(IEmap,aIEC(:,1));
...
patch2ply(interface,[IEmapPly, IEmapPly, IEmapPly],[pathName '\s_maps\' GBID '_IE_raw.ply'], limits);

%% export to *.ply  Cmap
minC = min(Cmap);
maxC = max(Cmap);
CAve = sum(Cmap.*aIEC(:,1))/AreaGB;
CErr = std(Cmap,aIEC(:,1));
...
patch2ply(interface,[CmapPly, CmapPly, CmapPly],[pathName '\s_maps\' GBID '_C_raw.ply'], limits);

%% export IE and C to *.txt
IEData = [pathName  '\values\' GBID '_data.txt'];
dataMatrix = [aIEC(:,1),aIEC(:,2),IEmap,aIEC(:,3),Cmap];
csvwrite(IEData,dataMatrix);

if str2double(GBID) == 0
%% visualising the results Cmap
   f4 = figure('Name','Interfacial excess map after smoothing','Visible','Off');
   trisurf(interface.faces,interface.vertices(:,1),interface.vertices(:,2),interface.vertices(:,3),IEmap);
   axis equal;
   rotate3d on;
   shading interp;
   c = colorbar;
   c.Label.String = ['Interfacial excess (' atomIon 's/nm^{2})'];
   fileNameOut = [pathName '\overview\s_IEmap.fig'];
   set(f4, 'CreateFcn', 'set(gcbo,''Visible'',''On'')');
   saveas (f4, fileNameOut);

%% visualising the results Cmap
   f5 = figure('Name','Grain boundary concentration map after smoothing','Visible','Off');
   trisurf(interface.faces,interface.vertices(:,1),interface.vertices(:,2),interface.vertices(:,3),Cmap);
   axis equal;
   rotate3d on;
   shading interp;
   c = colorbar;
   c.Label.String = ['Concentration (' atomIon '%)'];
   fileNameOut = [pathName '\overview\s_Cmap.fig'];
   set(f5, 'CreateFcn', 'set(gcbo,''Visible'',''On'')');
   saveas (f5, fileNameOut);
